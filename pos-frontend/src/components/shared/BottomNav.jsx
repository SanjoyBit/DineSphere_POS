...
const BottomNav = () => {
  ...
  // Set this value to true or false as needed
  const isAdmin = false;
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#1b1521] p-2 h-12 flex justify-around">
      ...
      {/* Conditional Button: Admin or Low Stock */}
      {isAdmin ? (
        <button
          onClick={() => navigate("/dashboard")}
          className="flex items-center justify-center font-bold text-[#ababab] w-[300px]"
        >
          <MdOutlineAdminPanelSettings className="inline mr-2" size={20} />
          <p>Admin</p>
        </button>
      ) : (
        <button
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title="Create Order">
        ...
        <div>
          ...
          <div className="flex items-center rounded-lg p-3 px-4 bg-[#3a2c4d] border border-[#6b5a8b]">
            <input
              ...
              className="bg-transparent [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none flex-1 text-white focus:outline-none"
            />
          </div>
        </div>
        <div>
        ...
        <button
          ...
          className="w-full bg-[#7d61cf] text-[#f5f5f5] rounded-lg py-3 mt-8 hover:bg-[#6b54b5]"
        >
          Create Order
        </button>
      </Modal>
